...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename(req, file, done) {
      ...
      done(null, basename + '_' + new Date().getTime() + '_' + ext);
    },
  }),
  limits: {
...
router.get('/:postId', async (req, res, next) => {
  try {
    ...
    if (!post) return res.status(403).send('존재하지않는 게시글입니다.');
...
router.post('/:postId/retweet', isLoggedIn, async (req, res, next) => {
  try {
    ...
    if (req.user.id === post.UserId || (post.Retweet && post.Retweet.UserId === req.user.id)) {
      return res.status(403).send('본인글은 리트윗할 수 없습니다.');
    }
...
router.post('/:postId/comment', isLoggedIn, async (req, res, next) => {
  try {
    ...
    const commentWithNickname = await Comment.findOne({
      where: { id: comment.id },
      include: [
        {
          model: User,
    ...
    res.status(201).json(commentWithNickname);
  } catch (error) {
    console.error(error);
    next(error);
  }
...
router.post('/images', isLoggedIn, upload.array('image'), async (req, res, next) => {
  try {
    ...
    res.status(201).json(req.files.map((el) => el.filename));
  } catch (error) {
    console.error(error);
    next(error);
  }
